...
submitButton4.addEventListener('click', (event) => {
    ...
    function calculateMaintenceCaloriesMetric() {
        const weight = parseFloat(document.querySelector('#weight1').value); // in kg
        const height = parseFloat(document.querySelector('#height1').value); // in cm
        ...
        document.querySelector("#calorie_info1").innerHTML = `
            ...
            <strong>Extreme Weight Gain:</strong> ${Math.ceil(maintenanceCalories + extremeWeightGain)} calories - 2 pounds a week<br>
        `;
    }
        
    calculateMaintenceCaloriesMetric();
});
